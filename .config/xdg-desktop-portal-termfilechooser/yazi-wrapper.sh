...
# Debug logging
logfile="/tmp/termfilechooser-debug.log"
echo "=== $(date) ===" >> "$logfile"
echo "multiple=$multiple" >> "$logfile"
...
if [ "$directory" = "0" ] && [ "$save" = "0" ] && [ -f "$out" ]; then
  tmpvalid=$(/usr/bin/mktemp)
  while IFS= read -r selected || [ -n "$selected" ]; do
    if [ -f "$selected" ]; then
      echo "$selected" >> "$tmpvalid"
    fi
  done < "$out"
  /usr/bin/mv "$tmpvalid" "$out"
fi
...
echo "=== output file contents ===" >> "$logfile"
cat "$out" >> "$logfile" 2>/dev/null || echo "(empty or missing)" >> "$logfile"
echo "" >> "$logfile"
